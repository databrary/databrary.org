...
import { KeycloakService } from '../server/src/keycloak/keycloak.service'
import { UserService } from '../server/dist/users/user.service'
import { AssetService } from '../server/src/asset/asset.service'
import { FileService } from '../server/src/file/file.service'
import { UserDTO } from '../server/dist/dtos/user.dto'
import { AssetDTO } from '../server/src/dtos/asset.dto'
import { FileDTO } from '../server/src/dtos/file.dto'
import { FileObjectDTO } from '../server/src/dtos/fileobject.dto'
...
async function main () {
  ...
  const pamSuccesses = await pMap(ingestData, async (pam) => {
    ...
    try {
      const pamAsset = await assetService.insertAsset(
        {
          id: pamId,
